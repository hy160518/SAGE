"""
Generates synthetic annotation data for SAGE evaluation.

This module creates realistic synthetic datasets including user roles,
relationships, chat messages, and ASR samples for evaluation purposes.
"""
...
class AnnotationGenerator:
    """Generates synthetic annotation data for evaluation."""
    ...
    def _generate_relationships(self) -> List[Dict[str, Any]]:
        """Generate user-drug relationships."""
        relationships = []
    ...
    def _generate_messages(self, user_roles: Dict[str, str]) -> List[Dict[str, Any]]:
        """Generate chat messages with drug mention indicators."""
        messages = []
        ...
        for msg_id in range(self.MESSAGES_PER_CASE):
            ...
            # Adjust drug mention probability by role
            if role == 'prescriber':
    ...
    def _generate_asr_samples(self) -> List[Dict[str, Any]]:
        """Generate ASR ground truth samples."""
        samples = []
        ...
        for sample_id in range(self.ASR_SAMPLES):
            ...
            # Simulated ASR output with errors
    ...
    def _introduce_errors(self, text: str, error_rate: float = 0.15) -> str:
        """Introduce random errors into text to simulate ASR mistakes."""
        chars = list(text)
        for _ in range(int(len(chars) * error_rate)):
            idx = random.randint(0, len(chars) - 1)
            if random.random() < 0.3:  # Deletion
                chars[idx] = ''
            elif random.random() < 0.6:  # Substitution
    ...
    def _generate_image_annotations(self) -> List[Dict[str, Any]]:
        """Generate image annotation data."""
